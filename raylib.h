...
RLAPI int somar(int a, int b);
RLAPI char* TruncateText(const char* text, int fontSize, int maxWidth);
int somar(int a, int b) {
    return a + b + b;
}
char* TruncateText(const char* text, int fontSize, int maxWidth) {
    if (text == NULL || strlen(text) == 0) return strdup("");

    // Mede a largura total do texto
    int textWidth = MeasureText(text, fontSize);

    // Se o texto já cabe na largura máxima, retorna uma cópia
    if (textWidth <= maxWidth) {
        return strdup(text);
    }

    // Se não cabe, precisamos truncar
    char* truncatedText = (char*)MemAlloc(strlen(text) + 4); // +4 para "..." e '\0'
    if (truncatedText == NULL) return strdup(""); // Falha na alocação

    // Texto para as reticências
    const char* ellipsis = "...";
    int ellipsisWidth = MeasureText(ellipsis, fontSize);

    // Itera para encontrar o ponto de corte
    int currentWidth = 0;
    int charCount = 0;
    for (int i = 0; i < strlen(text); i++) {
        char tempChar[2] = {text[i], '\0'}; // Caractere atual
        currentWidth += MeasureText(tempChar, fontSize);

        // Se adicionar o próximo caractere (mais as reticências) ultrapassar o limite,
        // então este é o ponto de corte.
    ...
    // Copia a parte truncada do texto
    strncpy(truncatedText, text, charCount);
    truncatedText[charCount] = '\0'; // Garante terminação nula

    // Adiciona as reticências
    strcat(truncatedText, ellipsis);
...
bool Fila_create(TFila *fila, int sizeElement, int max, int type)
{
  // verifica se paramentros da funao sao validos
  if(fila == NULL){
    return false;
  }
